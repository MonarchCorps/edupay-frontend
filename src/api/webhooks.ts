...
export async function getWebhookEvents(
    ...
    if (USE_MOCK) {
        ...
        if (params.virtualAccountId)
            list = list.filter(
                (w) => w.virtualAccountId === params.virtualAccountId,
            );
        if (params.eventType)
            list = list.filter((w) => w.eventType === params.eventType);
        if (params.processed !== undefined)
            list = list.filter((w) => w.processed === params.processed);
        return { data: list, total: list.length };
    }
    const { data } = await api.get<PaginatedResponse<WebhookEvent>>(
        '/webhooks/events',
        { params },
    );
    return data;
...
export async function replayWebhookEvent(
    ...
    const { data } = await api.post<WebhookEvent>(
