...
import { Card, Button, Select, SelectItem, TextInput, Flex } from '@tremor/react'
import { Plus, Search } from 'lucide-react'
import { PageHeader } from '../components/ui/PageHeader'
import { Modal } from '../components/ui/Modal'
import { AccountTable } from '../components/accounts/AccountTable'
import { ProvisionForm } from '../components/accounts/ProvisionForm'
import { useAccounts, useProvisionAccount, useFreezeAccount, useUnfreezeAccount, useCloseAccount } from '../hooks/useAccounts'
import { ACCOUNT_STATUSES, KYC_TIERS } from '../utils/constants'
import { useDebounce } from '../hooks/useDebounce'
import type { Account, AccountParams, AccountStatus, KycTier, ProvisionPayload } from '../types'
...
export default function Accounts() {
  ...
  const params = useMemo((): AccountParams => ({
  ...
  const handleProvision = useCallback(async (formData: ProvisionPayload) => {
    ...
    <div className="space-y-4">
      ...
      <Card>
        <Flex className="gap-3 flex-wrap mb-4">
          ...
          >
            ...
            {Object.entries(ACCOUNT_STATUSES).map(([k, { label }]) => (
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button color="red" loading={close.isPending} onClick={handleClose}>
              Close Account
            </Button>
          </>
        }
      >
        <p className="text-sm text-gray-600">
          ...
          <strong>{closeTarget?.customerName}</strong>? This action will sweep
          the remaining balance and cannot be undone.
        </p>
      </Modal>
    </div>
  )
