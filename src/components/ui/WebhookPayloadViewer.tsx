...
export function WebhookPayloadViewer({ payload, defaultOpen = false, className }: WebhookPayloadViewerProps) {
  const [open, setOpen] = useState(defaultOpen)
  const { copy, copied } = useCopyToClipboard()
  const json = JSON.stringify(payload, null, 2)

  return (
    <div className={clsx('rounded-tremor-default overflow-hidden border border-brand-dark/20', className)}>
      <div
        className="flex items-center justify-between px-4 py-2 bg-brand-dark cursor-pointer select-none"
        onClick={() => setOpen((o) => !o)}
      >
        <span className="text-brand-light text-sm font-mono font-medium">Payload</span>
        <div className="flex items-center gap-2">
          {open && (
            <button
              onClick={(e) => { e.stopPropagation(); copy(json) }}
